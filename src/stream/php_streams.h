...
#define php_stream_alloc_rel(ops, thisptr, persistent, mode) _php_stream_alloc((ops), (thisptr), (persistent), (mode))

#define php_stream_copy_to_mem_rel(src, buf, maxlen, persistent) _php_stream_copy_to_mem((src), (buf), (maxlen), (persistent))

#define php_stream_fopen_rel(filename, mode, opened, options) _php_stream_fopen((filename), (mode), (opened), (options))

#define php_stream_fopen_with_path_rel(filename, mode, path, opened, options) _php_stream_fopen_with_path((filename), (mode), (path), (opened), (options))

#define php_stream_fopen_from_fd_rel(fd, mode, persistent_id)	 _php_stream_fopen_from_fd((fd), (mode), (persistent_id))
#define php_stream_fopen_from_file_rel(file, mode)	 _php_stream_fopen_from_file((file), (mode))

#define php_stream_fopen_from_pipe_rel(file, mode)	 _php_stream_fopen_from_pipe((file), (mode))

#define php_stream_fopen_tmpfile_rel()	_php_stream_fopen_tmpfile(0)

#define php_stream_fopen_temporary_file_rel(dir, pfx, opened_path)	_php_stream_fopen_temporary_file((dir), (pfx), (opened_path))

#define php_stream_open_wrapper_rel(path, mode, options, opened) _php_stream_open_wrapper_ex((path), (mode), (options), (opened), NULL)
#define php_stream_open_wrapper_ex_rel(path, mode, options, opened, context) _php_stream_open_wrapper_ex((path), (mode), (options), (opened), (context))

#define php_stream_make_seekable_rel(origstream, newstream, flags) _php_stream_make_seekable((origstream), (newstream), (flags))
...
#define php_stream_alloc(ops, thisptr, persistent_id, mode)	_php_stream_alloc((ops), (thisptr), (persistent_id), (mode))
...
#define php_stream_from_zval_no_verify(xstr, ppzval)	(xstr) = (php_stream*)zend_fetch_resource((ppzval), -1, "stream", NULL, 2, php_file_le_stream(), php_file_le_pstream())

php_stream *php_stream_encloses(php_stream *enclosing, php_stream *enclosed);
#define php_stream_free_enclosed(stream_enclosed, close_options) _php_stream_free_enclosed((stream_enclosed), (close_options))
int _php_stream_free_enclosed(php_stream *stream_enclosed, int close_options);
...
char *php_stream_get_record(php_stream *stream, size_t maxlen, size_t *returned_len, const char *delim, size_t delim_len);

/* CAREFUL! this is equivalent to puts NOT fputs! */
int php_stream_puts(php_stream *stream, const char *buf);
...
php_stream *_php_stream_opendir(const char *path, int options, php_stream_context *context);
#define php_stream_opendir(path, options, context)	_php_stream_opendir((path), (options), (context))
...
int _php_stream_scandir(const char *dirname, char **namelist[], int flags, php_stream_context *context,
...
size_t _php_stream_copy_to_stream(php_stream *src, php_stream *dest, size_t maxlen STREAMS_DC);
#define php_stream_copy_to_stream(src, dest, maxlen)	_php_stream_copy_to_stream((src), (dest), (maxlen))
int _php_stream_copy_to_stream_ex(php_stream *src, php_stream *dest, size_t maxlen, size_t *len STREAMS_DC);
#define php_stream_copy_to_stream_ex(src, dest, maxlen, len)	_php_stream_copy_to_stream_ex((src), (dest), (maxlen), (len))
...
#define php_stream_copy_to_mem(src, buf, maxlen, persistent) _php_stream_copy_to_mem((src), (buf), (maxlen), (persistent))

/* output all data from a stream */
size_t _php_stream_passthru(php_stream * src STREAMS_DC);
...
void php_shutdown_stream_hashes();
/* PHP_RSHUTDOWN_FUNCTION(streams); */

int php_register_url_stream_wrapper(const char *protocol, php_stream_wrapper *wrapper);
int php_unregister_url_stream_wrapper(const char *protocol);
int php_register_url_stream_wrapper_volatile(const char *protocol, php_stream_wrapper *wrapper);
int php_unregister_url_stream_wrapper_volatile(const char *protocol);
php_stream *_php_stream_open_wrapper_ex(const char *path, const char *mode, int options, char **opened_path, php_stream_context *context STREAMS_DC);
php_stream_wrapper *php_stream_locate_url_wrapper(const char *path, const char **path_for_open, int options);
const char *php_stream_locate_eol(php_stream *stream, const char *buf, size_t buf_len);

#define php_stream_open_wrapper(path, mode, options, opened)	_php_stream_open_wrapper_ex((path), (mode), (options), (opened), NULL)
#define php_stream_open_wrapper_ex(path, mode, options, opened, context)	_php_stream_open_wrapper_ex((path), (mode), (options), (opened), (context))
...
int _php_stream_make_seekable(php_stream *origstream, php_stream **newstream, int flags STREAMS_DC);
#define php_stream_make_seekable(origstream, newstream, flags)	_php_stream_make_seekable((origstream), (newstream), (flags))

/* Give other modules access to the url_stream_wrappers_hash and stream_filters_hash */
/* HashTable *_php_stream_get_url_stream_wrappers_hash(); */
...
/* HashTable *_php_get_stream_filters_hash(); */
